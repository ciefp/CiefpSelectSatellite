#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpSelectSatellite/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpSelectSatellite

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpSelectSatellite
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpSelectSatellite
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpSelectSatellite/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpSelectSatellite-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#       CiefpSelectSatellite INSTALLED SUCCESSFULLY     #"
